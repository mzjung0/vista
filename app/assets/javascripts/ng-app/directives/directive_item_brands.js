...
klaseko.directive('itemBrandUpdate', ['Restangular', function(Restangular) {
  return {
    restrict: 'A',
    scope: {
      data: '=data'
    ...
    template: '<td><span>{{data.id}}</span></td>' +
              '<td><input type="text" ng-model=data.brand_code placeholder="Brand Code" /></td>' +
...
klaseko.directive('itemBrandAdd', ['Restangular', function(Restangular) {
  return {
    restrict: 'A',
    scope: {
      endpoint: '@'
