...
klaseko.directive('uomUpdate', ['Restangular', function(Restangular) {
  return {
    restrict: 'A',
    scope: {
      data: '=data'
    ...
    template: '<td><span>{{data.id}}</span></td>' +
              '<td><input type="text" ng-model=data.uom_code placeholder="UOM Code" /></td>' +       
...
klaseko.directive('uomAdd', ['Restangular', function(Restangular) {
  return {
    restrict: 'A',
    scope: {
      endpoint: '@'
