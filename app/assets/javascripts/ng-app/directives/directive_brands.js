...
klaseko.directive('brandUpdate', ['Restangular', function(Restangular) {
  return {
    restrict: 'A',
    scope: {
      data: '=data'
    ...
    template: '<td><span>{{data.id}}</span></td>' +
              '<td><input type="text" ng-model=data.brand_code /></td>' +
...
klaseko.directive('brandAdd', ['Restangular', function(Restangular) {
  return {
    restrict: 'A',
    scope: {
      endpoint: '@'
